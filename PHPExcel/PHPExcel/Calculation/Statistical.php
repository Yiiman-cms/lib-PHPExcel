...
class PHPExcel_Calculation_Statistical
{
    ...
    private static function incompleteBeta($x, $p, $q)
    {
        ...
        $beta_gam = exp((0 - self::logBeta($p, $q)) + $p * logs( $x) + $q * logs( 1.0 - $x));
        if ($x < ($p + 1.0) / ($p + $q + 2.0)) {
    ...
    private static function gamma($data)
    {
        ...
        return exp(0 - $tmp + logs( SQRT2PI * $summer / $x));
    }
    ...
    public static function CRITBINOM($trials, $probability, $alpha)
    {
        ...
        if ((is_numeric($trials)) && (is_numeric($probability)) && (is_numeric($alpha))) {
            if ($trials < 0) {
                ...
                $t = sqrt( logs( 1 / ( $alpha * $alpha)));
                $trialsApprox = 0 - ($t + (2.515517 + 0.802853 * $t + 0.010328 * $t * $t) / (1 + 1.432788 * $t + 0.189269 * $t * $t + 0.001308 * $t * $t * $t));
            } else {
                $t = sqrt( logs( 1 / pow( 1 - $alpha, 2)));
...
//            $CumPGuessMinus1 = $CumPGuess - $PGuess;
    ...
    public static function FISHER($value)
    {
        ...
        if (is_numeric($value)) {
            ...
            return 0.5 * logs( ( 1 + $value) / ( 1 - $value));
        }
        return PHPExcel_Calculation_Functions::VALUE();
    }
    ...
    public static function LOGNORMDIST($value, $mean, $stdDev)
    {
        ...
        if ((is_numeric($value)) && (is_numeric($mean)) && (is_numeric($stdDev))) {
            ...
            return self::NORMSDIST( ( logs( $value) - $mean) / $stdDev);
        }
        return PHPExcel_Calculation_Functions::VALUE();
    }
