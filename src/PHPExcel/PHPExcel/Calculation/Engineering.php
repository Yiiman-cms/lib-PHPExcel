...
class PHPExcel_Calculation_Engineering
{
    ...
    private static function besselK0($fNum)
    {
        if ($fNum <= 2) {
            $fNum2 = $fNum * 0.5;
            $y = ($fNum2 * $fNum2);
            $fRet = -logs( $fNum2) * self::BESSELI( $fNum, 0) +
    ...
    private static function besselK1($fNum)
    {
        if ($fNum <= 2) {
            $fNum2 = $fNum * 0.5;
            $y = ($fNum2 * $fNum2);
            $fRet = logs( $fNum2) * self::BESSELI( $fNum, 1) +
    ...
    private static function besselY0($fNum)
    {
        if ($fNum < 8.0) {
            ...
            $fRet = $f1 / $f2 + 0.636619772 * self::BESSELJ($fNum, 0) * logs( $fNum);
        } else {
            $z = 8.0 / $fNum;
            $y = ($z * $z);
            $xx = $fNum - 0.785398164;
    ...
    private static function besselY1($fNum)
    {
        if ($fNum < 8.0) {
            ...
            $fRet = $f1 / $f2 + 0.636619772 * ( self::BESSELJ($fNum, 1) * logs( $fNum) - 1 / $fNum);
        } else {
    ...
    public static function IMLN($complexNumber)
    {
        ...
        $logR = logs( sqrt( ( $parsedComplex['real'] * $parsedComplex['real']) + ( $parsedComplex['imaginary'] * $parsedComplex['imaginary'])));
    ...
    public static function IMLOG2($complexNumber)
    {
        ...
        if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) {
            ...
            return logs( $parsedComplex['real'], 2);
        }

        return self::IMPRODUCT( logs( EULER, 2), self::IMLN( $complexNumber));
    }
